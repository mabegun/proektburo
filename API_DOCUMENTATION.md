...
Глобальный поиск по всем сущностям системы. Вызывается при нажатии Ctrl+K или клике на кнопку поиска.

**Query params:**
- `q` (string, required) — поисковый запрос (минимум 2 символа)
- `types` (string, optional) — типы сущностей для поиска через запятую: `project,section,survey,task,employee`
- `limit` (number, optional) — максимальное количество результатов (по умолчанию 8)
...
[
  {
    ...
    "subtitle": "г. Москва, ул. Ленина, д. 45",
  ...
  },
  {
    ...
    "subtitle": "ЖК \"Северное сияние\" • В работе",
    "link": "/06-section-detail.html?id=uuid",
    "icon": "section"
  },
  {
    "id": "uuid",
    "type": "task",
    "title": "Проверить чертежи ГП",
    "subtitle": "ЖК \"Северное сияние\" • Высокий приоритет",
    "link": "/07-tasks-list.html?id=uuid",
    "icon": "task"
  },
  {
    ...
    "type": "employee",
    "title": "Иванов П.С.",
    "subtitle": "ГИП • 5 проектов",
    "link": "/09-employees-list.html?id=uuid",
    "icon": "employee"
  }
]
```
...
**Типы сущностей (type):**
| type | Описание | icon | link pattern |
|------|----------|------|--------------|
| `project` | Проект | project | `/04-project-detail.html?id={id}` |
| `section` | Раздел проекта | section | `/06-section-detail.html?id={id}` |
| `survey` | Изыскание | survey | `/14-survey-detail.html?id={id}` |
| `task` | Задача | task | `/07-tasks-list.html?id={id}` |
| `employee` | Сотрудник/подрядчик | employee | `/09-employees-list.html?id={id}` |

**Логика поиска:**
1. Поиск выполняется по названию, коду и описанию сущностей
2. Результаты сортируются по релевантности (точное совпадение > начало строки > вхождение)
3. Приоритет: проекты > разделы > задачи > сотрудники
4. Учитываются права доступа пользователя (возвращаются только доступные сущности)
5. Debounce на frontend — 300ms после ввода последнего символа

**Используется в:** `js/pages/dashboard.js` (функции `initSearch`, `search`, `renderSearchResults`)
